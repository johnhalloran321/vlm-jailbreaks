...
def main() -> None:
    ...
    parser.add_argument("--batch-root", type=Path, default=Path("results/attacks/visual_cipher"), help="Root directory containing per-row outputs.")
    parser.add_argument(
        "--output-file",
        type=str,
        default="",
    ...
    with args.csv.open(newline="", encoding="utf-8") as f:
        ...
        for idx, row in enumerate(reader):
            ...
            for vlm_path in vlm_paths:
                if args.vlm_pattern and args.vlm_pattern not in vlm_path.name:
                    continue
    ...
    def worker(task):
        ...
        try:
            ...
            if out_path.exists():
                return None  # Skip silently or return a message if verbose
