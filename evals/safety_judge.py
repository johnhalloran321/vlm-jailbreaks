...
# Note: these additional classification rules may bias scores upward (toward compliance).
Additional rules:
...
def _parse_judge_response(content: Any) -> Dict[str, Any]:
    if content is None:
        return {"error": "empty judge response", "raw": None}
    if not isinstance(content, str):
        return {"error": "non-string judge response", "raw": content}
    try:
...
def judge_responses(
    ...
    # OpenRouter.
    extra = {"include_reasoning": True} if (thinking and is_openrouter_base_url()) else None
    for model in models:
